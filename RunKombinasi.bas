...
Dim GlobalAvgPercentageBefore As Double
Dim GlobalAvgPercentageAfter As Double
...
Sub submit_multiple_replacement3()
    ...
    GlobalPercentageAfter = tempAfter
     ...
     WSsim.Range("D27").Value = GlobalAvgPercentageBefore ' Total Sustainability Before?
    WSsim.Range("E8").Value = GlobalAvgPercentageBefore
    WSsim.Range("E9").Value = GlobalAvgPercentageBefore
    WSsim.Range("E10").Value = GlobalAvgPercentageBefore
    WSsim.Range("D28").Value = GlobalAvgPercentageAfter  ' Total Sustainability After
...
Sub CalculateSustainabilityBefore()
    ...
    If GlobalTotalBefore > 0 Then
        ...
        If validSheetCount > 0 Then
            GlobalAvgPercentageBefore = totalPersentaseSustainabilityBefore / validSheetCount
        Else
    ...
    For Each namaKategori In totalPersenPerKategori.Keys
        For i = 1 To 6
            If sustainableCategories.Exists(namaKategori) Then
                avgKategori = totalPersenPerKategori(namaKategori) / countPerKategori(namaKategori)
...
Sub CalculateSustainabilityAfter()
    ...
    If GlobalTotalAfter > 0 Then
        ...
        If validSheetCount > 0 Then
            GlobalAvgPercentageAfter = totalPersentaseSustainabilityAfter / validSheetCount
        Else
    ...
    For i = 1 To arrDel.Count
        ...
        For rr = lastRow To 3 Step -1 ' dari bawah ke atas
            If Trim(UCase(ws.Cells(rr, "H").Value)) = Trim(UCase(nm)) Then
                If IsNumeric(ws.Cells(rr, "I").Value) Then
                    ws.Rows(rr).ClearContents
                End If
                Exit For
            End If
        Next rr
...
Sub update_multiple_material_data_with_backup(replacements() As replacementData)
    ...
    For Each wsName In specSheets
        ...
        ' Hitung total material pada spesifikasi ini (kolom J)
        ...
        For keyIndex = 1 To materialKeys.Count
            ...
            If materialExists Then
                ...
                Dim idxNew1 As Long
              ' Saat update existing material:
For idxNew1 = LBound(replacements) To UBound(replacements)
    If UCase(replacements(idxNew1).new_material) = UCase(materialName) Then
        If totalSpecWeight > 0 Then
...
Next idxNew1
            ...
            Else
                ...
                If classFoundRow > 0 Then
                    ...
                    Dim idxNew2 As Long
                    ' Saat menambah material baru:
For idxNew2 = LBound(replacements) To UBound(replacements)
    If UCase(replacements(idxNew2).new_material) = UCase(materialName) Then
        If totalSpecWeight > 0 Then
...
DebugLog totalallportion
    ...
    For i = 1 To x
        ...
        Call submit_multiple_replacement3
        simResults(i) = GlobalAvgPercentageAfter
        simResults(i) = GlobalAvgPercentageAfter
